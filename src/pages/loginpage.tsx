...
export default function LoginPage() {
  ...
  const [googleLoading, setGoogleLoading] = useState(false); // Separate loading state
  const { login, googleSignIn } = useAuth();
  const navigate = useNavigate();

  const from = '/dashboard'; // Fixed usage of location state
      ...
      <div className="w-full max-w-md">
        <div className="bg-card rounded-lg shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" disabled={loading || googleLoading} className="w-full bg-primary text-white py-3 rounded-lg font-medium hover:bg-primary/90 disabled:opacity-50">
          ...
          <button 
            ...
            className="w-full bg-white text-gray-900 border border-gray-300 py-3 rounded-lg font-medium hover:bg-gray-50 flex items-center justify-center gap-2 disabled:opacity-50 transition-colors"
          >
            {googleLoading ? (
